    ...
    const handleSave = async () => {
        try {
            if (isNew) {
                ...
                const response = await axiosInstance.post('/accounts/add-account', editedAccount);
                setRows([...rows, response.data]);  // Update table with new data
            } else {
                // Update an existing account
                await axiosInstance.put(`/accounts/update-account/${selectedAccount.account_name}`, editedAccount);
