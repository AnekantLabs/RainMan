...
def _establish_connection(subaccount_name: str, credentials: Dict[str, str]) -> bool:
    ...
    try:
        ...
        def wallet_callback(message):
            logger.info(f"Wallet message: {message}")
            if message.get("op") == "pong":
                logger.debug(f"Pong received from {subaccount_name}: {message}")
            # Handle other wallet messages...

        def order_callback(message):
            logger.info(f"Order message: {message}")
            if message.get("op") == "pong":
                logger.debug(f"Pong received from {subaccount_name}: {message}")
            # Handle other order messages...
        ...
        # Start ping thread to send heartbeat
        def send_ping():
            while True:
                try:
                    ...
                    raw_ws = ws.ws  # Access low-level websocket
                    if raw_ws and raw_ws.sock and raw_ws.sock.connected:
                        raw_ws.send(json.dumps({
                            "req_id": "ping_heartbeat",
                            "op": "ping"
                        }))
                        logger.info(f"Sent ping to {subaccount_name}")
                    else:
                        logger.warning(f"WebSocket not connected for {subaccount_name}")
                        break

                    sleep(20)  # Send ping every 20 seconds
...
def connect_websocket_startup():
    try:
        ...
        if accounts_data:
            ...
            print("⚠️ No active accounts")
    except Exception as e:
        print(f"❌ Failed during websocket startup: {e}")
